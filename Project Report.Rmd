...
Our Histone Specific heatmap (Figure 6) provides a strong replication of the paper's Figure 1C, showing a robust and uniform up regulation of histone family transcripts (such as H2bc21 and H2ax) in the MEL-R replicates. To achieve this we searched the significant gene list for all candidate genes that matched the following regex: "\^H[0-9]\|hist", as well as the mentioned genes from the paper. Noticeably, the heatmaps differ on a substantial portion (top) where we see a large blue region for the HISAT2 pipeline. Curiously, one sample for the Salmon pipeline exhibited down regulation where the other replicates for the MEL-R samples showed up regulation. Fundamentally we see that the induction of resistance leads to a strong up regulation of histone genes, confirming the previous findings.
...
```{r, echo=FALSE, fig.cap="Gene-concept network (cnetplot) of downregulated pathways. Network visualization showing connections between the significantly enriched GO terms and their associated differentially expressed genes, comparing HISAT (left) and Salmon (right) results. Network shows that Salmon pipeline results are driven by histone genes whereas the HISAT pipeline identifies clusters related to cellular structure and development.", out.width="110%", fig.align="center"}

knitr::include_graphics("Combined Plots/Gene_Network_Down.png")
```

As shown in the Downregulated cnetplot (Figure 9, right), the results seem to be driven by a tightly interconnected cluster of histone-family genes (e.g., H1f2, H1f0, H2bc21). Therefore, the transcript-level resolution of the Salmon pipeline is more sensitive to downregulation of specific histone genes, compared to the HISAT2 alignment, which may mask them at the gene-level. On the HISAT2 plot we see many genes found by the authors to cause "cytoskeletal collapse" where the MEL cells are loosing the genes required for cillium organization and assembly as well eye development.
...
The HISAT2 upregulated network (Figure 10, Left) is extremely dense in connections. Genes such as Was, Btk, Rac2, Spi1, Dock2, Vav1, Ptprc, and Itgb2 connect multiple immune and cell activation pathways, indicating these components could be part of a signaling program that defines MEL cell identity. We see the affects of the upregulation could likely be causing an immune cell lineage change, as a result of induced resistance.

## Conclusion

The re-analysis of the MEL and MEL-R datasets confirms the biological findings from Fernández-Calleja et al., 2017. Despite modernizing every component of the data processing pipeline, the core results stayed quite consistent. Actin cytoskeletal genes (Was, Btk, Rac2, Dock2, and Plek) are downregulated in differentiation-resistant MEL-R cells, while histone genes are upregulated. The key biological process in the original study (collapse of the actin cytoskeletal network and remodeling of the epigenetic structure) stayed consistent in the re-analysis.

The difference in pathways found between the two pipelines do suggest that pipeline construction must be customized to the dataset being used. Salmon excels at capturing more specific pathways from the RNA-Seq data, while Figures 9 and 10 highlight HISAT2's ability to identify richer gene networks, which could be much more useful for identifying signal processes. Both HISAT2 and Salmon illustrated clear separation of MEL and MEL-R in PCA and had a few overlapping sets of differentially expressed genes. But, HISAT2 had a very concentrated variance (99%) in PC1, while the Salmon variance was distributed more evenly (39% in PC1). The 2 pipelines had different genes driving differentiation, with HISAT2 being dominated by Car1 and Pgk1 and Salmon having Rac2 and Rassf4 on the top. Salmon uses probabilistic mapping at the transcription level, which could redistribute the feature counts more evenly. On the other hand, HISAT2 deterministically counts each read mapping to a gene's exons, which could cause certain genes to have a dominating variance.
...
GEO Database: <http://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=GSE83567>
